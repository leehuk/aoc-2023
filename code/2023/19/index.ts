        ...
        if(match) {
            ...
            for(let el of match[2].split(",")) {
                ...
                if(ematch) {
                    ...
                    res.rules[rname].push({ name: rname, action: el } as Rule);
                }
            }
        }
...
function handleOne(filename: string, check?: Object): void {
    ...
    for(let part of data.parts) {
        ...
        for(let i = 0; i < rules.length; i++) {
            let rule = rules[i];
                if(rule.comp === undefined || (rule.comp == "<" && part[rule.field as keyof Part] < rule.val!) || (rule.comp == ">" && part[rule.field as keyof Part] > rule.val!)) {
                    if(rule.action == "A") {
                        accept.push(part);
                        continue OUTER;
                    } else if(rule.action == "R") {
